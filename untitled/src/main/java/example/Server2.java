...
public class Server2 {
    ...
    private static final int serverId = 2;       // Server3 için ID

    public static void main(String[] args) {
        int myClientPort = 6002; // İstemcilerle iletişim için port
        int myServerPort = 5002; // Sunucular arası iletişim için port
        startListeningFromAdmin(7002);
        int[] otherServerPorts = {5001, 5003}; // Diğer sunucuların portları
        int adminPort = 8002; // Admin ile Capacity iletişimi için port
    ...
    private static void connectToOtherServers(int[] ports, List<Subscriber> subscribers) {
        ...
        for (int port : ports) {
            int CURRENT_PORT = 5002; // Mevcut sunucunun portu
                        ...
                        try (DataInputStream dis = new DataInputStream(clientSocket.getInputStream())) {
                            ...
                            synchronized (subscribers) {
                                if (faultToleranceLevel == 1 && serverId == 3) {
                                    // Server3 bağımsız liste kullanıyorsa sadece kendi listesini günceller
                                    updateSubscriberList(subscribers, subscriber);
                                } else if (faultToleranceLevel >= 2) {
                                    // Tüm serverlar shared list kullanıyorsa listeyi günceller
                                    updateSubscriberList(subscribers, subscriber);
                                } else {
                                    // Bağımsız liste
                                    updateSubscriberList(subscribers, subscriber);
    ...
    private static void broadcastToOtherServers(Subscriber subscriber) {
        int[] otherServerPorts = {5001, 5003}; // Diğer sunucuların portları (Server2 hariç)
