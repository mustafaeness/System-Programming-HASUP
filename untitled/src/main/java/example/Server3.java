...
public class Server3 {
    ...
    private static final int serverId = 3;       // Server3 için ID
    public static void main(String[] args) {
        int myClientPort = 6003; // İstemcilerle iletişim için port
        int myServerPort = 5003; // Sunucular arası iletişim için port
        startListeningFromAdmin(7003);
        int[] otherServerPorts = {5001, 5002}; // Diğer sunucuların portları
        int adminPort = 8003; // Admin ile Capacity iletişimi için port
        ...
        startListeningFromClients(myClientPort, subscribers, faultToleranceLevel, serverId);
    ...
    private static void connectToOtherServers(int[] ports, List<Subscriber> subscribers) {
        ...
        for (int port : ports) {
            int CURRENT_PORT = 5003; // Mevcut sunucunun portu
    ...
    private static void startListeningFromClients(int port, List<Subscriber> subscribers, int faultToleranceLevel, int serverId) {
        new Thread(() -> {
                        ...
                        try (DataInputStream dis = new DataInputStream(clientSocket.getInputStream())) {
                            ...
                            synchronized (subscribers) {
                                ...
                                if (faultToleranceLevel == 1 && serverId == 3) {
                                    System.out.println("Fault Tolerance Level is 1: Server3 cannot modify the list.");
                                    return; // Server3 listeyi güncelleyemez
                                }
    ...
    private static void updateSubscriberList(List<Subscriber> subscribers, Subscriber newSubscriber) {
        synchronized (subscribers) {
            boolean updated = false;

            // Aboneyi mevcut listede bulup güncelle
            for (int i = 0; i < subscribers.size(); i++) {
                if (subscribers.get(i).getId() == newSubscriber.getId()) {
                    subscribers.set(i, newSubscriber);
                    updated = true;
                    System.out.println("Subscriber updated: " + newSubscriber);
                    break;
                }
            }

            // Eğer abone bulunamazsa listeye ekle
            if (!updated) {
                subscribers.add(newSubscriber);
                System.out.println("Subscriber added: " + newSubscriber);
            }
        }
    }
    ...
    private static void broadcastToOtherServers(Subscriber subscriber) {
        int[] otherServerPorts = {5001, 5002}; // Diğer sunucuların portları (Server3 hariç)
    ...
    private static void handleConnection(Socket socket, List<Subscriber> subscribers) {
        try (DataInputStream dis = new DataInputStream(socket.getInputStream())) {
            ...
            synchronized (subscribers) {
                boolean updated = false;
                if (faultToleranceLevel == 1 && serverId == 3) {
                    // Fault Tolerance Level 1 ise ve bu Server3 ise bağımsız liste kontrol edilir
                    if (!subscribers.stream().anyMatch(s -> s.getId() == subscriber.getId())) {
                        System.out.println("Error: Subscriber with ID " + subscriber.getId() + " not found in Server3.");
                        return;
                    }
                } else {
                    // Diğer durumlarda liste güncellenir
                    updateSubscriberList(subscribers, subscriber);
                }
            }
            // Güncel listeyi logla
            logSubscribers(subscribers);
        } catch (Exception e) {
